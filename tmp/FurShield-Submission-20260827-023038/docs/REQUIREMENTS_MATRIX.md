...
| Adopter interest and final status | Owner interest forms, shelter replies and decision status, persisted in-app notifications, and optional Resend email delivery |
...
Optional family account sharing and AI chatbot are not enabled. Payment, physical delivery, and veterinarian credential verification are explicitly excluded by the SRS. Application email delivery requires optional `RESEND_API_KEY`/`EMAIL_FROM` configuration; in-app alerts work without it. A hosted URL and mandatory MP4 demonstration are release artifacts, not locally verifiable application features.
