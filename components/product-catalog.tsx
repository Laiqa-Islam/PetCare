...
export function ProductCatalog({items=demoProducts}:{items?:CatalogProduct[]}) {
  ...
  return <>
    ...
    <div className="section-heading compact-heading"><div className="filter-bar" aria-label="Product categories">{categories.map((item) => <button className={`filter-chip ${item === category ? "active" : ""}`} key={item} onClick={() => setCategory(item)}>{item}</button>)}</div><div className="cart-summary" id="cart"><BagIcon /><strong>{count} items</strong><span>${total.toFixed(2)}</span></div></div>
    <div className="card-grid">{filtered.map((item) => <article className="catalog-card" key={item.id}>
      ...
      <div className="catalog-body"><div className="meta-row"><span>{item.category} · {item.pet}</span><span>★ {item.rating}</span></div><h2>{item.name}</h2><p>{item.stock} in stock · Product ordering is recorded without payment or delivery.</p><div className="meta-row"><span className="price">${item.price.toFixed(2)}</span><span>{cart[item.id] ? `Qty ${cart[item.id]}` : "Not in cart"}</span></div><div className="card-actions"><button className="button button-primary" onClick={() => setCart((current) => ({...current,[item.id]:(current[item.id] ?? 0)+1}))}>Add to cart</button>{cart[item.id] ? <button className="button button-ghost" aria-label={`Remove one ${item.name}`} onClick={() => setCart((current) => ({...current,[item.id]:Math.max(0,(current[item.id] ?? 0)-1)}))}>Remove</button> : null}</div></div>
    </article>)}</div>
  </>;
}
